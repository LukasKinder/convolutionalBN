...
/* float *** gibbsSampling(ConvolutionalBayesianNetwork cpm, float ** startingImage, int n_samples, int iterations, float smoothing_factor){

    setStateToImage(cpm,startingImage);
    ...
    prob_before = probabilityState(cpm);
    ...
    for (int i = 1; i < n_samples; i++){
        ...
        for (int j = 0; j < iterations / n_samples; j++){
            ...
            setStateToImage(cpm,currentImage);

            prob_after = probabilityState(cpm);
...
float *** simulatedAnnealing(ConvolutionalBayesianNetwork cpm, int n_samples, float ** startingImage, int n_iterations){
...
float *** strictClimbing(ConvolutionalBayesianNetwork cpm, int n_samples, float ** startingImage, int n_iterations){
...
float obstructionTask(ConvolutionalBayesianNetwork cpm, float ** startingImage, int n_iterations, int obstructionSize){
...
/* void saveBestWorst(bool *** images, int n_images, int n_best ,ConvolutionalBayesianNetwork cpm){
    float * image_probs = malloc(sizeof(float) * n_images);
    for(int i = 0; i < n_images; i++){
        setStateToImage(cpm,images[i]);
        image_probs[i] = logProbabilityStatecpm(cpm); //TODO UPDATE, in order to implement depenent
    }
