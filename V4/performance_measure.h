...
void predictionNumberOneLayer(ConvolutionalBayesianNetwork cpm, int layer,  float * prob_dist){
    BayesianNetwork bn = cpm->bayesianNetworks[layer];
    NumberNode nn;
    for (int i = 0; i < 10; i++){
        prob_dist[i] = 0;
    }
...
void predictNumber(ConvolutionalBayesianNetwork cpm, int *prediction1, int * prediction2, bool verbose ){
    float * overall_prob_dist = calloc(sizeof(float), 10);
    float * local_prob_dist = malloc(sizeof(float) * 10);

    int n_layer_with_nn = 0;
    for (int i = 0; i < cpm->n_layers; i++){

        if (cpm->bayesianNetworks[i]->n_numberNodes == 0){
            continue;
        }
        n_layer_with_nn++;


        predictionNumberOneLayer(cpm, i,local_prob_dist);
    ...
    for (int i = 0; i < cpm->n_layers; i++){
        n_number_nodes += cpm->bayesianNetworks[i]->n_numberNodes;
    }
...
/* void predictNumberManualExperiment(ConvolutionalBayesianNetwork cpm, float *** images, int n_images, int * labels){
    ...
    for (int i = 0; i < 30; i++){
        ...
        setStateToImage(cpm,images[random_image]);
        printImageContinuos(images[random_image],28);
        printf("is %d, predicts %d\n", labels[random_image], predictNumber(cpm,true));

    }
} */

float predictNumberAccuracy(ConvolutionalBayesianNetwork cpm, char* image_path, char * label_path, int n_data, bool verbose){
    ...
    for (int i = 0; i < n_data; i++){
        ...
        setStateToImage(cpm, images[i]);
        predictNumber(cpm,&guess1, &guess2,false);
        if (guess1== labels[i] ){
            accuracy +=1;
        }
...
int canFindBestRotation(ConvolutionalBayesianNetwork cpm, float ** image, bool verbose){
    ...
    for (int i = 0; i < 4;i++){
        setStateToImage(cpm,image);
        this = average_probability_nodes(cpm->bayesianNetworks[1]);
        //this = logLikelihoodState(cpm->bayesianNetworks[1]);
...
float rotatingImageAccuracy(ConvolutionalBayesianNetwork cpm, char* image_path, char * pathLabels, int n_data, bool verbose){
    ...
    ConvolutionalBayesianNetwork * copys_cpm = malloc(sizeof(ConvolutionalBayesianNetwork) * N_THREADS);
    for (int i =0 ; i < N_THREADS; i++){
        copys_cpm[i] = copyConvolutionalBayesianNetwork(cpm);
    }
    ...
    for (int i = 0; i < n_data; i++){
        ...
        direction = canFindBestRotation(copys_cpm[omp_get_thread_num()],images[i],false);
    ...
    for (int i =0 ; i < N_THREADS; i++){
        freeConvolutionalBayesianNetwork(copys_cpm[i]);
    }
    free(copys_cpm);
