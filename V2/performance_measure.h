...
void predictNumberManualExperiment(ConvolutionalBayesianNetwork cbn, bool *** images, int n_images, int * labels){
    ...
    for (int i = 0; i < 30; i++){
        ...
        printImage(images[random_image],28);
...
float predictNumberAccuracy(ConvolutionalBayesianNetwork cbn, char* image_path, char * label_path, int n_data, bool verbose){
    ...
    bool *** images = readImages(image_path,n_data,0.3);
